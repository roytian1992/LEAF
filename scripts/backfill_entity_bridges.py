...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backfill entity bridge snapshots from existing LEAF events.")
    parser.add_argument("--config", required=True)
    parser.add_argument("--db", required=True)
    parser.add_argument("--corpus-id", action="append", default=[])
    parser.add_argument("--mode", choices=["hybrid", "embedding_cluster", "graph_lexical"], default="hybrid")
...
def main() -> None:
    ...
    try:
        ...
        for corpus_id in corpus_ids:
            result = service.backfill_entity_bridges(
                corpus_id=corpus_id,
